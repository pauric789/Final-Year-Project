...
class ShotDetector:
    ...
    def run(self):
        while True:
            ...
            # Data tracking for CSV export
            self.record_frame_data(frame_ball_pos, frame_hoop_pos)
    ...
    def record_frame_data(self, ball_pos, hoop_pos):
        """Record all tracking data for this frame"""
        frame_record = {
        ...
        # Add all pose landmarks
        for lm_name in [lm.name for lm in self.pose_landmarks_needed]:
            if lm_name in self.pose_points:
                x, y, visibility = self.pose_points[lm_name]
                frame_record[f'{lm_name}_x'] = x
                frame_record[f'{lm_name}_y'] = y
                frame_record[f'{lm_name}_visibility'] = visibility
            else:
                frame_record[f'{lm_name}_x'] = None
                frame_record[f'{lm_name}_y'] = None
                frame_record[f'{lm_name}_visibility'] = None
        
        # Add shot state
        frame_record['in_up_region'] = self.up
        frame_record['in_down_region'] = self.down
        frame_record['pending_shot'] = self.pending_shot
        frame_record['shot_cooldown'] = self.shot_cooldown
        
        self.frame_data.append(frame_record)
        
        # If tracking a shot, add to current shot frames
        if self.tracking_shot:
            self.current_shot_frames.append(frame_record)
